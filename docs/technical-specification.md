...
_Last reviewed: 2026-04-16 · Version 3.0 (Build 31)_

> **Recent platform changes (audit pass 2026-04-16):** the page chrome (header,
> footer, admin shells) is now provided by the shared `erikr/chrome` Composer
> library — `inc/html_header.php` and `inc/html_footer.php` are thin wrappers
> over `\Erikr\Chrome\Header::render()` / `\Erikr\Chrome\Footer::render()`.
> Account-security actions (password change, TOTP 2FA enrol/disable) live in
> `web/security.php`; `web/preferences.php` is display/UX preferences only.
> Runtime config now comes from a generated `config.yaml` (not `config/db.json`)
> — see `mcp/generate.py` in the deploy ecosystem.
...
vendor/erikr/auth/      ← Shared auth library (Composer, symlinked)
  ...
  ├── src/admin.php     ← Admin user CRUD (admin_create_user, admin_delete_user, …)
  ├── src/totp.php      ← TOTP 2FA helpers
  ├── src/avatar.php    ← Avatar upload + serving
  └── src/mailer.php    ← send_mail() wrapper (PHPMailer)

vendor/erikr/chrome/    ← Shared page-chrome library (Composer, symlinked)
  ├── src/Header.php    ← \Erikr\Chrome\Header::render()
  ├── src/Footer.php    ← \Erikr\Chrome\Footer::render()
  └── src/Admin/        ← UsersTab, LogTab, UserModals, Dispatch (admin §15)

config.yaml             ← DB + SMTP + deploy config (generated by mcp/generate.py)
...
│   ├── index.php           Main monitor SPA shell
│   ├── api.php             Unified JSON API
...
│   ├── security.php        Account security (password change, TOTP enrol/disable)
...
│   ├── ogd.php             OGD CSV import + view rebuild
│   └── yaml.php            Tiny YAML loader for config.yaml
├── config.yaml             Active runtime config (generated; gitignored)
├── config.example.yaml     Template
├── data/                   Runtime state (gitignored)
│   └── ratelimit.json
├── migrations/             Numbered SQL migrations (NN_*.sql)
├── vendor/                 Composer deps (erikr/auth, erikr/chrome, phpmailer, …)
├── scripts/                CLI utilities (deploy.sh, update_ogd.php, migrate.php, …)
...
### 4.6 setpassword.php — Initial password setup (invite flow)

New accounts created via the admin "Benutzer anlegen" modal receive an
invitation email with a one-time token. `setpassword.php` validates the token
(via `erikr/auth`'s `invite_*` helpers), shows a "set your initial password"
form, and on success activates the account and logs the user in. Replaces the
old self-registration `activate.php` flow.
...
### 4.9 preferences.php — Display / UX preferences

Requires login. Display- and UX-only settings. Handles four POST actions via a
hidden `action` field:
...
**Note:** password change is **not** here — it lives in `web/security.php`
together with TOTP 2FA enrol/disable. Do not re-introduce password forms into
preferences.php.

---

### 4.9a security.php — Account security

Requires login. The single canonical place for account-security actions:

| Action | What it does |
|---|---|
| `change_password` | Verifies old password, hashes new (cost 13) via `auth_change_password()`, updates DB |
| `totp_start` | Generates a TOTP secret, stages it in session, renders QR + manual code |
| `totp_confirm` | Validates the 6-digit code, persists the secret to `auth_accounts.totp_secret` |
| `totp_disable` | Clears the user's TOTP secret (with confirm prompt) |
...
Interactive script that delegates to the shared `mcp/deploy.py` coordinator
in `~/Git/mcp/`. Targets are declared in `config.yaml` (`target: local |
akadbrain | world4you`) so a single deploy understands which DB, SMTP, and
base URL to use. `rsync --copy-links` resolves the `vendor/erikr/*` symlinks
into real files at the destination. `config.yaml` and `data/` are excluded
from the sync — they live independently per environment.

### 17.3 Environment detection in PHP

`inc/initialize.php` reads `config.yaml` directly (`wl_yaml_load()`) and
defines `APP_ENV` from its top-level `target:` field. There is no Apache
`SetEnv`; the active environment is determined entirely by which `config.yaml`
is on disk.
...
`mcp/generate.py --app wlmonitor --target <local|akadbrain|world4you>`). Shape:
...
A starter template lives in `config.example.yaml`. Never commit a populated
`config.yaml` — credentials are environment-specific.

**`inc/initialize.php` constants (selected):**
...
```json
{
  ...
  "require": { "erikr/auth": "*", "erikr/chrome": "*" },
...
Path-repo Composer dep at `vendor/erikr/auth` (symlink to `~/Git/auth`).
Provides via `autoload.files`:
- `auth_bootstrap()`, `auth_require()`, `admin_require()`, `auth_login()`, `auth_logout()`, `auth_change_password()`
- `csrf_token()`, `csrf_input()`, `csrf_verify()`
- `rate_limit_check()`, `rate_limit_record()`, `rate_limit_clear()`, `auth_blacklist_*()`
- `admin_create_user()`, `admin_edit_user()`, `admin_delete_user()`, `admin_reset_password()`, `admin_register_delete_cleanup()`
- `auth_totp_*()` — enrol, confirm, disable, complete
- `appendLog()`, `addAlert()`, `getUserIpAddr()`
- `invite_*()` — invite tokens for admin-created accounts
- `send_mail()` (PHPMailer wrapper), `avatar_*()` helpers

Auth logic is **not** in scope for this project. Flag issues as recommendations
for the auth library; do not patch them in wlmonitor.

**Runtime: `erikr/chrome`**

Path-repo Composer dep at `vendor/erikr/chrome` (symlink to `~/Git/chrome`).
Provides:
- `\Erikr\Chrome\Header::render([...])` — fixed app header (UI rule §12)
- `\Erikr\Chrome\Footer::render([...])` — fixed three-column footer (UI rule §13)
- `\Erikr\Chrome\Admin\{UsersTab, LogTab, UserModals, Users, Dispatch}` — canonical admin §15.x implementation

**Dev: `phpunit/phpunit ^13.0`**

`tests/` (Unit + Integration). Run via `make test` or `vendor/bin/phpunit`.
Not deployed (excluded by rsync).
