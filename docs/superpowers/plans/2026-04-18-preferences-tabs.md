# Preferences Tabs Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Reorganise `preferences.php` into five hash-synced tabs (Design, Abfahrten, E-Mail, Sicherheit, Profilbild), absorbing all of `security.php`'s logic, and retire `security.php` to a redirect.

**Architecture:** `preferences.php` gains the password-change and TOTP handlers from `security.php`. The HTML is restructured around the shared `.tab-bar` / `.tab-btn` / `.tab-panel` components with client-side hash routing. `security.php` becomes a one-line redirect; the user-menu link is updated to point at `preferences.php#sicherheit`.

**Tech Stack:** PHP 8.2, vanilla JS, shared CSS component library (`tab-bar`, `tab-btn`, `tab-panel` from `components.css`), `chillerlan/qrcode` (already composer-required by `security.php`).
...
| File | Change |
|---|---|
| `web/preferences.php` | Full rewrite — new PHP header + tabbed HTML |
| `web/security.php` | Replace with redirect |
| `inc/html_header.php` | Update `securityHref` value |
| `inc/initialize.php` | Bump `APP_BUILD` |

---

## Task 1: Rewrite `web/preferences.php` PHP header

Replaces lines 1–153 (current PHP + opening HTML) with the merged logic from both files.

**Files:**
- Modify: `web/preferences.php`

- [ ] **Step 1: Replace the entire PHP block (lines 1–153) with the merged version**

Open `web/preferences.php` and replace everything from `<?php` through `$avatarUrl = 'avatar.php?id=' . $userId;` with:
...
Expected: `No syntax errors detected in web/preferences.php`

- [ ] **Step 3: Commit**

```bash
git add web/preferences.php
git commit -m "feat(prefs): absorb security.php handlers into preferences.php"
```

---

## Task 2: Rewrite `web/preferences.php` HTML as tabbed layout

Replaces everything from `<?php include_once(__DIR__ . '/../inc/html_header.php'); ?>` to the end of the file.

**Files:**
- Modify: `web/preferences.php`

- [ ] **Step 1: Replace the HTML portion (from the `include html_header` line to end of file) with the tabbed layout**
...
- [ ] **Step 3: Open the dev site and verify all five tabs render and switch correctly**

Open `http://localhost/wlmonitor.test/preferences.php` in a browser.

Check:
- All five tab buttons appear
- Clicking each tab shows the correct panel and hides the others
- Navigating to `preferences.php#sicherheit` directly opens on the Sicherheit tab
- The Design radio buttons reflect the current theme
- The Abfahrten slider reflects the current departures setting
- The E-Mail tab shows the current email address
- The Sicherheit tab shows either the password form + 2FA status correctly
- The Profilbild tab shows the avatar

- [ ] **Step 4: Commit**

```bash
git add web/preferences.php
git commit -m "feat(prefs): tabbed layout — Design, Abfahrten, E-Mail, Sicherheit, Profilbild"
```

---

## Task 3: Retire `security.php` + update header link + bump build

**Files:**
- Modify: `web/security.php`
- Modify: `inc/html_header.php`
- Modify: `inc/initialize.php`

- [ ] **Step 1: Replace `web/security.php` with a redirect**

Replace the entire contents of `web/security.php` with:
...
- [ ] **Step 2: Update the `securityHref` in `inc/html_header.php`**
...
- [ ] **Step 3: Bump APP_BUILD in `inc/initialize.php`**
...
- [ ] **Step 4: Verify `security.php` redirects correctly**

Open `http://localhost/wlmonitor.test/security.php` — the browser should land on `preferences.php#sicherheit` with the Sicherheit tab active.

- [ ] **Step 5: Verify the user-menu "Passwort & 2FA" link opens the Sicherheit tab**

Open the user-menu dropdown. Click "Passwort & 2FA". Confirm it lands on `preferences.php#sicherheit`.

- [ ] **Step 6: Commit**

```bash
git add web/security.php inc/html_header.php inc/initialize.php
git commit -m "feat(prefs): retire security.php; route Passwort & 2FA to preferences#sicherheit"
```
