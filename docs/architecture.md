...
> - **Frontend stack** is the shared CSS library at `~/Git/css` (symlinked
...
| Frontend     | Shared CSS library (`~/Git/css`, symlinked into `web/css/shared/`) + vanilla JS (ES modules) |
...
│   ├── css/
│   │   ├── shared/             Symlink → ~/Git/css (theme, reset, layout, components, icons)
...
| `AUTH_DATABASE_NAME`| Auth DB name (defaults to `jardyx_auth`)                 |
...
1. Check rate limit.
2. Look up user in `jardyx_auth.auth_accounts`.
...
- **`jardyx_auth`** (shared with other Jardyx apps) — managed by the
...
```bash
# Auth schema (run once per host that hosts jardyx_auth)
